...
public class Spawner072 : MonoBehaviour
{
    [SerializeField]
    ...
    public static Spawner072 instance;
    ...
    void Update()
    {
        if (hasGameFinished || !hasGameStarted) return; // Jika permainan telah selesai atau belum dimulai, keluar dari fungsi ini
        if (Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.Space))
        {
            if (stack.Count > 1)
                stack[stack.Count - 1].GetComponent<Tile>().ScaleTile(); // Panggil fungsi ScaleTile pada ubin terakhir dalam tumpukan
            if (hasGameFinished) return; // Jika permainan telah selesai setelah memanggil ScaleTile, keluar dari fungsi ini
    ...
    void CreateTile()
    {
        ...
        // Mengatur posisi ubin baru di atas ubin sebelumnya
        ...
        activeTile.GetComponent<Tile>().moveDirection = (stack.Count - 1) % 4; // Mengatur arah gerakan ubin baru
    }
